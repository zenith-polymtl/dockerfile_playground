    ...
    async def process_web_command(self, data):
        try:
            ...
            if message['type'] == 'command':
                ...
                if command in self.control_publishers:  # Changed to use control_publishers directly
                    msg = String() if not command.endswith('bucket_number') else Int32()
                    msg.data = data if data else command.split('/')[-1].upper()
                    self.control_publishers[command.split('/')[-1]].publish(msg)
                    await self.send_terminal_message(f"Command executed: {command}")
                else:
